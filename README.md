...
## API-слой
 ...
 ## `StoreApi` - сервис для работы с API магазина
...
## Модели данных
 ...
 - `close` - генерирует событие CLOSE.
  ...
  ## `OrderModel` - Модель данных заказа, реализует хранение данных address и способа оплаты, email и телефона.
 ...
 - `getValidOrder` - проверяет, заполнены ли поля способа оплаты и адреса;
 - `getValidContacts` - проверяет, заполнены ли поля email и телефона;
 - `getErrorOrder` - возвращает первую подходящую ошибку типа string если не выбран способ оплаты и не заполнено поле с адресом;
 - `getErrorContacts` - возвращает первую подходящую ошибку типа string если не заполнены поля email или телефона;
...
## `ITotal` - хранит данные об успешном заказе
 ...
 - `total: number` - общая стоимость товаров в корзине.
  ...
  ## `IOrderChange` - интерфейс данных заказа.
